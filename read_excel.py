"""Модуль реализует загруку тестовых данных из экселя в бд.
 Принцип работы заключается в том, что мы читаем каждый sheet в excel,
 из него формируем название таблицы транслитом.
 Определяем типы данные в листе, для того чтобы автоматически создавать таблцы.
 После создания таблицы, подаём на запись список кортежей, который записывается в бд."""
